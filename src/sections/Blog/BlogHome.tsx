...
export default function BlogHome({
  ...
  // @ts-ignore injected by loader
  query = "",
}: Props & { currentPage?: number; query?: string }) {
  const all = posts ?? [];
  ...
  const prevHref = page - 1 <= 1 ? paginationBase || "/" : `${paginationBase}?page=${page - 1}`;
          ...
          <div className="max-w-[1280px] mx-auto px-[clamp(1rem,3vw,2rem)] py-12 md:py-16">
            ...
            {query && (
              <p className="text-[#7a7a74] text-base leading-relaxed max-w-[520px]">
                {all.length} {all.length === 1 ? "resultado" : "resultados"} para &ldquo;<strong>{query}</strong>&rdquo;
              </p>
            )}
          </div>
        </div>
        ...
        <div className="border-b border-[#e4e3df]">
          ...
          >
            ...
            <div className="flex flex-col justify-center py-12 md:py-16 px-6 md:pr-14 md:pl-[clamp(1.5rem,5vw,4rem)] order-2 md:order-1 bg-white">
              ...
              {heroPost.date && (
                <span className="text-xs text-[#a0a09a]">{formatDate(heroPost.date)}</span>
              )}
            </div>
            {/* Right — image */}
            <div className="relative overflow-hidden aspect-[4/3] md:aspect-auto md:[min-height:clamp(440px,66vh,680px)] order-1 md:order-2 bg-[#f3f2ee]">
              {heroPost.image && (
                <img
                  src={heroPost.image}
                  alt={(heroPost as any).alt || heroPost.title}
                  loading="eager"
                  decoding="async"
                  fetchPriority="high"
              ...
              <div
                ...
                style={{ height: 220, background: "linear-gradient(to bottom,rgba(0,0,0,0.55) 0%,rgba(0,0,0,0.18) 55%,transparent 100%)", zIndex: 1 }}
              />
            </div>
          </a>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            {duoPosts.map((post, i) => (
              <a
                key={post.slug}
                href={`/${post.slug}`}
                className={`blog-reveal flex flex-col gap-5 p-6 md:p-10 group no-underline text-inherit${i === 0 ? " border-b md:border-b-0 md:border-r border-[#e4e3df]" : ""}`}
              ...
              >
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {post.excerpt && (
                    <p className="text-[#7a7a74] text-sm leading-relaxed line-clamp-3">{post.excerpt}</p>
                  )}
                  {post.date && (
                    <span className="text-xs text-[#a0a09a] mt-1">{formatDate(post.date)}</span>
                  )}
                </div>
              </a>
            ))}
      ...
      <div className="max-w-[1400px] mx-auto px-[clamp(1rem,3vw,2rem)] pb-10">
        {listPosts.length === 0 && (
          <div className="py-20 flex flex-col items-center text-center gap-4">
            ...
            <a href="/" className="text-[11px] font-medium tracking-[0.12em] uppercase text-[#ff6011] no-underline hover:opacity-75 transition-opacity">
        ...
        {listPosts.map((post, i) => (
          <a
            key={post.slug}
            href={`/${post.slug}`}
            className={`blog-reveal flex items-start gap-6 py-8 group no-underline text-inherit${i < listPosts.length - 1 ? " border-b border-[#e4e3df]" : ""}`}
          ...
          >
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-1.5">
              ...
              {post.excerpt && (
                <p className="text-[#7a7a74] text-sm leading-relaxed line-clamp-2">{post.excerpt}</p>
              )}
              {post.date && (
                <span className="text-xs text-[#a0a09a] mt-1">{formatDate(post.date)}</span>
              )}
        ...
        <div className="flex justify-center items-center gap-4 pb-16">
          {hasPrev ? (
            <a
              href={prevHref}
              rel="prev"
              onClick={(e) => { e.preventDefault(); window.location.assign(prevHref); }}
            ...
            <span className="px-5 py-2.5 text-[11px] font-semibold tracking-[0.1em] uppercase text-[#c0bfbb] select-none">← Anterior</span>
          )}

          <span className="text-[11px] text-[#a0a09a] tabular-nums">{page} / {totalPages}</span>

          {hasNext ? (
            <a
              href={nextHref}
              rel="next"
              onClick={(e) => { e.preventDefault(); window.location.assign(nextHref); }}
              className="flex items-center gap-2 px-5 py-2.5 border border-[#e4e3df] text-[11px] font-semibold tracking-[0.1em] uppercase text-[#1a1a18] hover:bg-[#1a1a18] hover:text-white hover:border-[#1a1a18] transition-colors duration-150 no-underline"
            >
              Próxima →
            </a>
          ) : (
            <span className="px-5 py-2.5 text-[11px] font-semibold tracking-[0.1em] uppercase text-[#c0bfbb] select-none">Próxima →</span>
          )}
        </div>
      )}

      <script defer dangerouslySetInnerHTML={{ __html: getRevealScript(containerId) }} />
    </div>
  );
}
